...
function updateExpenseList() {
    ...
    ex.innerHTML=`<h2 >${heading.textContent} &#8377;${expenseSum}</h2>
    ...
    expenses.forEach((expense, index) => {
        ...
        listItem.innerHTML = `
            <span>Expense ${index + 1}: &#8377;${expense.valu}</span>
            <br>
            <span>${expense.description}<span>
            <span style="float:right;">
            <button onclick="editExpense(${index})"><i class="material-icons" style="font-size:20px;" >edit</i></button>
            <button onclick="deleteExpense(${index})"><i class="material-icons" style="color:red; font-size:20px;">delete</i></button></span>`
